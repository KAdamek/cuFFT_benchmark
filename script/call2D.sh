...
do
rm timing-${ID}-${TYPE}-${LENGTH}-${LENGTH}-0.txt

#run the logger on the energy profiling
	nvidia-smi --query-gpu=timestamp,power.draw,clocks.current.sm,clocks.current.memory --format=csv,noheader,nounits -i $CARD -f nvidiasmi-${ID}-${TYPE}-${LENGTH}-${LENGTH}-0.txt -lms 10 &
	PID=$(echo $!)
...
#        echo "Text read from file: $line"
	...
	nvprof --print-gpu-trace -u ms --csv ../cuFFT_benchmark.exe ${LENGTH} ${LENGTH} 0 -2 200 f ${TYPE} $CARD 2>out.nvprof
	...
	sleep 5
	echo ${TIMESTAMP}"," ${START_TIME}"," ${END_TIME}"," ${END_PLUS_TIME}"," ${CORE_MEM}>> timing-${ID}-${TYPE}-${LENGTH}-${LENGTH}-0.txt
done < "$1"
