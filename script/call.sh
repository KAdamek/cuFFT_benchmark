...
#run the logger on the energy profiling
	nvidia-smi --query-gpu=timestamp,power.draw,clocks.current.sm,clocks.current.memory --format=csv,noheader,nounits -i $CARD -f huh.txt -lms 10 &
	PID=$(echo $!)
...
#        echo "Text read from file: $line"
	...
	nvprof --print-gpu-trace -u ms --csv ../cuFFT_benchmark.exe 2097152 0 0 -2 200 f R2C $CARD 2>out.nvprof
	...
	sleep 5
	echo ${TIMESTAMP}"," ${START_TIME}"," ${END_TIME}"," ${END_PLUS_TIME} >> timing.txt
done < "$1"
