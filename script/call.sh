...
#run the logger on the energy profiling
	nvidia-smi --query-gpu=timestamp,power.draw,clocks.current.sm,clocks.current.memory --format=csv,noheader,nounits -i $CARD -f nvidiasmi-${ID}-${PREC}-${TYPE}-${LENGTH}-0-0.txt -lms 10 &
	PID=$(echo $!)
